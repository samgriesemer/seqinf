...
class BNPE_C(PosteriorEstimator):
    def __init__(
        self,
    ...
    def _log_prob_proposal_posterior_atomic(
        ...
        BNPE NOTE: Main change for now is the batch handling of the atoms when
        calling ``_neural_net.log_prob``, keeping the same component model for
        each atom. I'm unsure whether this has a clear effect or really matters
        in any way, but it feels like a degree of consistency that might make
        more sense that random masks across all points in all atoms. May even
        consider a fix component model for all atoms in a given epoch.
